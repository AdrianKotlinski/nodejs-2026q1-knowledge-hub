...
export function validateTranslateResult(raw: unknown): TranslateResult {
  ...
  return {
    translatedText:
      typeof obj?.translatedText === 'string' ? obj.translatedText : String(raw),
    detectedLanguage:
      typeof obj?.detectedLanguage === 'string' ? obj.detectedLanguage : 'unknown',
  };
}
...
export function validateAnalyzeResult(raw: unknown): AnalyzeResult {
  ...
  return {
    ...
    suggestions: Array.isArray(obj?.suggestions)
      ? (obj.suggestions as unknown[]).filter((s) => typeof s === 'string') as string[]
      : ['Review the content manually'],
    severity: validSeverities.includes(obj?.severity as (typeof validSeverities)[number])
